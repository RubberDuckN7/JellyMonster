    ...
    public class WizardTypeData : ContentObject
    {
		...
		public class WizardTypeDataReader : ContentTypeReader<WizardTypeData>
        {
                ...
                WizardTypeData existingInstance)
            {
                ...
                desc.Wizard_Attacking.AddRange(input.ReadObject<List<string>>());
                desc.Wizard_Waiting.AddRange(input.ReadObject<List<string>>());
                desc.Magic_Missile = input.ReadString();
                desc.Extents = input.ReadObject<Vector2>();
                desc.Magic_Speed = input.ReadSingle();
