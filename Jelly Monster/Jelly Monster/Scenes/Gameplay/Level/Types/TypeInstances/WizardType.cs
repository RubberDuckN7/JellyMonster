...
{
    public class WizardType : EnemyType
    {
        ...
        {
            WizardTypeData data = content.Load<WizardTypeData>(path);

            animation = new Animation((byte)data.Wizard_Waiting.Count);

            for (byte i = 0; i < animation.Count; i++)
            {
                Textures[i] = content.Load<Texture2D>(data.Wizard_Waiting[i]);
            }

            attack_animation = new Animation((byte)data.Wizard_Attacking.Count);

            for (byte i = 0; i < attack_animation.Count; i++)
            {
                attack_animation[i] = content.Load<Texture2D>(data.Wizard_Attacking[i]);
            }

            bullet = content.Load<Texture2D>(data.Magic_Missile);

            extents = data.Extents;
            bullet_speed = data.Magic_Speed;
            fire_rate = data.Fire_Rate;
        }

        public override void Update(Level level, EntityInterface entity, float dt)
        {
            Wizard enemy = entity as Wizard;
