...
{
    public class WorldSceneFive : IScene
    {
        ...
        public WorldSceneFive(PlayerData player_data, PlayerSaveData player_save)
        {
            this.player_data = player_data;
            this.player_save = player_save;
            this.level_id = 254;

            this.player_data.world_id = 4;
        ...
        public override void Load()
        {
            ...
            background = content.Load<Texture2D>("Textures/background_industrial");
            ...
            levels = SceneManager.Content.Load<MapData>("XML/Worlds/MapFive");

            missions = Page.AddContainer(EORIENTATION.Right, 65, 25, 7, 3, 25f, 25f, 25f, 25f, 60f, 25f);
            missions.Event += OnMissions;

            for (byte i = 0; i < 21; i++)
            {
                if (player_save.scores.Count > i + 84)
                {
        ...
        public override void Draw(SpriteBatch sp)
        {
            ...
            //sp.DrawString(Fonts.FontMenu, "World five", Vector2.Zero, Color.Red);
            ...
            if (SceneState == IrisEngine.SceneState.Active && player_data.world_id == 4)
            {
        ...
        public override void Update(float dt, bool has_focus, bool covered_by_other)
        {
            ...
            if (GamePad.GetState(PlayerIndex.One).Buttons.Back == ButtonState.Pressed)
            {
                if (!BackButton.pressed)
                {
                    ...
                    ExitScene();
                    SceneManager.AddScene(new WorldSceneFour(player_data, player_save));
                }
            }
            else
        ...
        public void OnNext(byte id, TouchLocationState state)
        {
            if (state == TouchLocationState.Released)
            {
                SoundManager.PlayOnClick();
                ExitScene();
                SceneManager.AddScene(new WorldSceneSix(player_data, player_save));
            }
        }
    }
}
