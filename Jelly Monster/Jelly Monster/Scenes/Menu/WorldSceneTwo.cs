...
{
    public class WorldSceneTwo : IScene
    {
        ...
        public WorldSceneTwo(PlayerData player_data, PlayerSaveData player_save)
        {
            this.player_data = player_data;
            this.player_save = player_save;
            this.level_id = 254;

            this.player_data.world_id = 1;
        ...
        public override void Load()
        {
            ...
            background = content.Load<Texture2D>("Textures/background_underworld");
            ...
            levels = SceneManager.Content.Load<MapData>("XML/Worlds/MapTwo");
            ...
            for (byte i = 0; i < 21; i++)
            {
                if (player_save.scores.Count > i + 21)
                {
            ...
            ButtonNamed b_next = Page.AddButton(new Rectangle(600, 380, 200, 100), EORIENTATION.Bottom, "Next", 0);
            ButtonNamed b_play;
        ...
        public override void Draw(SpriteBatch sp)
        {
            ...
            //sp.DrawString(Fonts.FontMenu, "World two", Vector2.Zero, Color.Red);
            ...
            if (SceneState == IrisEngine.SceneState.Active && player_data.world_id == 1)
            {
                for (byte i = 0; i < items.Length; i++)
                {
                    if (items[i].id < 200 && i + 21 < player_save.scores.Count)
                    {
                        ...
                        sp.DrawString(Fonts.FontMessage, "" + player_save.scores[items[i].id + 21], p, Color.Gold, 0f, Vector2.Zero, 1f, SpriteEffects.None, 1f);
                    }
                }
            }
        ...
        public override void Update(float dt, bool has_focus, bool covered_by_other)
        {
            ...
            if (GamePad.GetState(PlayerIndex.One).Buttons.Back == ButtonState.Pressed)
            {
                if (!BackButton.pressed)
                {
                    ...
                    ExitScene();
                    SceneManager.AddScene(new WorldSceneOne(player_data, player_save));
                }
            }
            else
        ...
        public void OnPlay(byte id, TouchLocationState state)
        {
            if (state == TouchLocationState.Pressed)
            {
                ...
                if(player_data.trial)
                    ...
                    //LoadingScene.Load(SceneManager, true, new GameScene(player_data, player_save, levels.Levels[level_id + 21]));
            }
        }
        ...
        public void OnMissions(byte id, TouchLocationState state)
        {
            ...
            this.player_data.current_id += 21;
        ...
        public void OnNext(byte id, TouchLocationState state)
        {
            if (state == TouchLocationState.Released)
            {
                SoundManager.PlayOnClick();
                ExitScene();
                SceneManager.AddScene(new WorldSceneThree(player_data, player_save));
            }
        }
    }
}
